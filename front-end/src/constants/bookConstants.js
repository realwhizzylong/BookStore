export const BOOK_LIST_SUCCESS = 'BOOK_LIST_SUCCESS';
export const BOOK_LIST_FAIL = 'BOOK_LIST_FAIL';

export const BOOK_DETAILS_SUCCESS = 'BOOK_DETAILS_SUCCESS';
export const BOOK_DETAILS_FAIL = 'BOOK_DETAILS_FAIL';

export const BOOK_CREATE_SUCCESS = 'BOOK_CREATE_SUCCESS';
export const BOOK_CREATE_FAIL = 'BOOK_CREATE_FAIL';
export const BOOK_CREATE_RESET = 'BOOK_CREATE_RESET';

export const BOOK_EDIT_SUCCESS = 'BOOK_EDIT_SUCCESS';
export const BOOK_EDIT_FAIL = 'BOOK_EDIT_FAIL';
export const BOOK_EDIT_RESET = 'BOOK_EDIT_RESET';

export const BOOK_DELETE_SUCCESS = 'BOOK_DELETE_SUCCESS';
export const BOOK_DELETE_FAIL = 'BOOK_DELETE_FAIL';

export const MY_BOOKS_SUCCESS = 'MY_BOOKS_SUCCESS';
export const MY_BOOKS_FAIL = 'MY_BOOKS_FAIL';
export const MY_BOOKS_RESET = 'MY_BOOKS_RESET';

export const BOOK_REVIEW_SUCCESS = 'BOOK_REVIEW_SUCCESS';
export const BOOK_REVIEW_FAIL = 'BOOK_REVIEW_FAIL';
export const BOOK_REVIEW_RESET = 'BOOK_REVIEW_RESET';

export const BOOK_SOLD_SUCCESS = 'BOOK_SOLD_SUCCESS';
export const BOOK_SOLD_FAIL = 'BOOK_SOLD_FAIL';
export const BOOK_SOLD_RESET = 'BOOK_SOLD_RESET';